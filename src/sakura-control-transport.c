...
sakura_control_request_clear(SakuraControlRequest *request)
{
	...
	request->cols = 0;
	request->rows = 0;
	request->terminal_kind = SAKURA_TAB_SHELL;
...
gboolean
sakura_control_encode_set_task_status_request(const gchar *request_id,
	                                           const gchar *task_id,
	                                           guint status,
	                                           GByteArray *payload)
{
	Sakura__Control__V1__SetTaskStatusRequest set_status =
		SAKURA__CONTROL__V1__SET_TASK_STATUS_REQUEST__INIT;
	Sakura__Control__V1__Request request = SAKURA__CONTROL__V1__REQUEST__INIT;

	if (payload == NULL || request_id == NULL || request_id[0] == '\0' ||
	    task_id == NULL || task_id[0] == '\0')
		return FALSE;
	set_status.task_id = (gchar *)task_id;
	set_status.status = status;
	request.request_id = (gchar *)request_id;
	request.body_case = SAKURA__CONTROL__V1__REQUEST__BODY_SET_TASK_STATUS;
	request.set_task_status = &set_status;
	return sakura_control_pack_message(&request.base, payload);
}
...
gboolean
sakura_control_encode_restart_terminal_request_with_spec(
	                                               const gchar *request_id,
	                                               const gchar *terminal_id,
	                                               const gchar *page_id,
	                                               const gchar *group_id,
...
sakura_control_encode_restart_terminal_request_with_page(
	                                               ...
	                                               GByteArray *payload)
{
	return sakura_control_encode_restart_terminal_request_with_spec(
		request_id, terminal_id, page_id, group_id, task_id, cwd, cols, rows,
		SAKURA_TAB_SHELL, NULL, NULL, NULL, payload);
}
	                            ...
	                            GError **error)
{
	...
	case SAKURA__CONTROL__V1__REQUEST__BODY_SET_PAGE_ARCHIVED:
		...
		break;
	case SAKURA__CONTROL__V1__REQUEST__BODY_SET_TASK_STATUS:
		if (decoded->set_task_status != NULL) {
			request->kind = SAKURA_CONTROL_REQUEST_SET_TASK_STATUS;
			request->task_id = g_strdup(decoded->set_task_status->task_id);
			request->status = decoded->set_task_status->status;
		}
		break;
	...
	case SAKURA__CONTROL__V1__REQUEST__BODY_RESTART_TERMINAL:
		if (decoded->restart_terminal != NULL) {
			...
			request->terminal_kind = decoded->restart_terminal->kind;
			request->resume_session_id = g_strdup(
	                            ...
	                            const SakuraCoreTerminal *terminal)
{
	...
	message->codex_session_name = (gchar *)sakura_control_string(
		terminal->codex_session_name);
	message->codex_session_name_set_by_user =
		terminal->codex_session_name_set_by_user;
	message->resume_on_start = terminal->resume_on_start;
	...
	guint64 sequence, SakuraSessionSnapshot **snapshot, GError **error)
{
	...
	*snapshot = NULL;
	...
	for (gsize index = 0; index < wire_snapshot->n_terminals; index++) {
		Sakura__Control__V1__Terminal *wire_terminal =
			wire_snapshot->terminals[index];
		SakuraSessionTabRecord *tab = g_new0(SakuraSessionTabRecord, 1);

		tab->terminal_id = g_strdup(wire_terminal->id);
		tab->parent_id = g_strdup(wire_terminal->task_id != NULL &&
		                          wire_terminal->task_id[0] != '\0'
		...
		tab->codex_session_name_set_by_user =
			wire_terminal->codex_session_name_set_by_user;
